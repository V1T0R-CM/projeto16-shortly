...
export async function postShorten(req, res) {
    try {
        ...
        const { rows : users } = await connection.query(`SELECT id FROM users WHERE email = '${jwt.verify(token, jwtSecretKey).email}'`);
        connection.query(`INSERT INTO url ("userId", "shortUrl", "fullUrl") VALUES ('${users[0].id}', '${shortUrl}', '${req.body.url}')`)
